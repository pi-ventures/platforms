...
const MODEL_CONFIG = {
  claude: {
    label: 'Claude', subLabel: 'by Anthropic',
    color: 'from-purple-600 to-indigo-600', hoverShadow: 'hover:shadow-purple-200',
    badge: 'bg-purple-100 text-purple-700', loadingText: 'Claude is reading your',
    dot: 'bg-purple-400', iconBg: 'bg-purple-600',
    envHint: 'Add ANTHROPIC_API_KEY=sk-ant-... to your .env.local file and restart the dev server',
    logo: (<svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor"><path d="M13.827 3.52h-3.654L5.551 20h3.651l4.625-16.48zm2.957 0-4.625 16.48h3.652L20.449 3.52h-3.665z"/></svg>),
  },
  openai: {
    label: 'ChatGPT', subLabel: 'GPT-4o by OpenAI',
    color: 'from-emerald-500 to-teal-600', hoverShadow: 'hover:shadow-emerald-200',
    badge: 'bg-emerald-100 text-emerald-700', loadingText: 'ChatGPT is reading your',
    dot: 'bg-emerald-400', iconBg: 'bg-emerald-600',
...
export default function AITutorPage() {
  ...
  const toggleVoice = () => {
    ...
    recognition.onresult = (e: any) => setQuestion(prev => prev ? prev + ' ' + e.results[0][0].transcript : e.results[0][0].transcript)
  ...
  const captureCamera = () => {
    ...
    setSelectedBook(null); setResponse(null); setError(null)
    closeCamera()
  }
  ...
  const captureScreen = async () => {
    ...
    } catch (err: any) { if (err.name !== 'AbortError' && err.name !== 'NotAllowedError') console.error(err) }
  }
  ...
  const handleSelectionStart = (e: React.MouseEvent) => {
    ...
    setDragging(true); setSelection(null)
  }
  ...
  const handleSelectionEnd = () => {
    ...
    const closeOverlay = () => { setScreenSelectOpen(false); setScreenshot(null); setDragStart(null); setDragCurrent(null); setSelection(null); setSelectedBook(null); setResponse(null); setError(null) }
    if (w < 10 || h < 10) { setImage(screenshot); setImageFileName('Full screen capture'); closeOverlay(); return }
    const canvas = document.createElement('canvas')
    canvas.width = Math.round(w * scaleX); canvas.height = Math.round(h * scaleY)
    const src = new Image()
    src.onload = () => { canvas.getContext('2d')!.drawImage(src, Math.round(x1 * scaleX), Math.round(y1 * scaleY), canvas.width, canvas.height, 0, 0, canvas.width, canvas.height); setImage(canvas.toDataURL('image/png')); setImageFileName('Screen capture (region)'); closeOverlay() }
    src.src = screenshot
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl overflow-hidden w-full max-w-lg shadow-2xl">
            ...
            <div className="p-4 flex gap-3">
              ...
              <button onClick={captureCamera} className="flex-1 bg-purple-600 text-white py-2.5 rounded-xl text-sm font-bold hover:bg-purple-700 flex items-center justify-center gap-2">
      ...
      {screenSelectOpen && screenshot && (
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 bg-black/60 text-white flex-shrink-0">
            <div><p className="text-sm font-semibold">Select a Region</p><p className="text-xs text-white/50 mt-0.5">Drag to crop · Click to use full screenshot</p></div>
            <button onClick={() => { setScreenSelectOpen(false); setScreenshot(null); setDragStart(null); setDragCurrent(null); setSelection(null) }} className="p-1.5 hover:bg-white/10 rounded-lg"><X className="w-4 h-4" /></button>
          </div>
          <div className="flex-1 overflow-auto flex items-center justify-center p-6 cursor-crosshair" onMouseDown={handleSelectionStart} onMouseMove={handleSelectionMove} onMouseUp={handleSelectionEnd}>
            <div className="relative inline-block select-none">
              <img ref={screenshotImgRef} src={screenshot} className="block rounded-lg shadow-2xl" draggable={false} style={{ maxWidth: '100%', maxHeight: 'calc(100vh - 120px)' }} alt="Screen capture" />
              {selection && selection.w > 2 && selection.h > 2 && (
                <div className="absolute border-2 border-blue-400 bg-blue-400/10 pointer-events-none" style={{ left: selection.x, top: selection.y, width: selection.w, height: selection.h }} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl flex flex-col shadow-2xl" style={{ maxHeight: '80vh' }}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 flex-shrink-0">
              <p className="font-bold text-gray-900">Select from Library</p>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2"><Brain className="w-7 h-7 text-purple-600" /> AI Tutor</h1>
          <p className="text-gray-500 mt-1">Pick a book from the library, capture your screen, or just type</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <button onClick={() => setModelDropdownOpen(o => !o)} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:border-gray-300 hover:bg-gray-50 transition-all shadow-sm">
                ...
                <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden">
                  {(['claude', 'openai'] as AIModel[]).map(m => (
                    <button key={m} onClick={() => { setSelectedModel(m); setModelDropdownOpen(false) }}
                      ...
                      <span className={selectedModel === m ? (m === 'claude' ? 'text-purple-600' : 'text-emerald-600') : 'text-gray-500'}>{MODEL_CONFIG[m].logo}</span>
      ...
      {!response && !error && (
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm mb-6">
          ...
          {/* Context selector — library book or captured image */}
          {selectedBook ? (
            ...
            <button onClick={() => setLibraryOpen(true)}
              ...
              <p className="text-gray-700 font-medium text-sm">Browse Library</p>
              <p className="text-gray-400 text-xs mt-0.5">Pick a textbook or reference book</p>
            </button>
          )}

          {/* Secondary: Camera / Screen / Paste — shown when no context selected */}
          {!image && !selectedBook && (
          ...
          <div className="relative mb-1">
            <textarea value={question} onChange={e => setQuestion(e.target.value)}
              ...
              placeholder={isListening ? 'Listening…' : 'Type your question or tap the mic to speak'}
              rows={3}
          ...
          <button onClick={handleAsk} disabled={loading || (!question.trim() && !image && !selectedBook)}
            ...
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" />Analysing with {cfg.label}…</> : <><Send className="w-4 h-4" />Ask {cfg.label}</>}
          </button>
        </div>
      )}
      ...
      {loading && (
        <div className="bg-white rounded-2xl p-6 border border-purple-100 shadow-sm mb-6 flex items-center gap-4">
          ...
          {selectedBook && !image && (
            <div className="w-16 h-16 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: subjectColor(selectedBook.subject) + '15' }}>
              <BookOpen className="w-7 h-7" style={{ color: subjectColor(selectedBook.subject) }} />
            </div>
          )}
          <div>
            <p className="font-semibold text-gray-900 text-sm">{cfg.loadingText} {image ? 'image' : selectedBook ? 'textbook' : 'question'}…</p>
            <p className="text-xs text-gray-400 mt-1">Preparing explanation</p>
            <div className="flex gap-1 mt-2">{[0,1,2].map(i => <div key={i} className={`w-2 h-2 rounded-full ${cfg.dot} animate-bounce`} style={{ animationDelay: `${i * 0.15}s` }} />)}</div>
          </div>
        </div>
      )}
      ...
      {response && (
        <div className="space-y-4">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${responseCfg.badge}`}>
            <span>{responseCfg.logo}</span> Answered by {responseCfg.label} ({responseCfg.subLabel})
          </div>

          {(image || selectedBook) && (
            <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-center gap-3">
              {image
                ? <img src={image} alt="Your question" className="w-14 h-14 object-cover rounded-xl border border-gray-200" />
                : selectedBook && (
                  <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: subjectColor(selectedBook.subject) + '15' }}>
                    <BookOpen className="w-7 h-7" style={{ color: subjectColor(selectedBook.subject) }} />
                  </div>
                )
              }
              <div>
                <p className="text-xs font-semibold text-gray-500">Your question</p>
                <p className="text-sm text-gray-600">{selectedBook ? selectedBook.title : imageFileName}</p>
          ...
          <div className={`bg-gradient-to-br ${usedModel === 'claude' ? 'from-purple-50 to-indigo-50 border-purple-100' : 'from-emerald-50 to-teal-50 border-emerald-100'} rounded-2xl p-6 border`}>
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-8 h-8 ${responseCfg.iconBg} rounded-xl flex items-center justify-center text-white`}><Brain className="w-4 h-4" /></div>
              <h3 className={`font-bold ${usedModel === 'claude' ? 'text-purple-900' : 'text-emerald-900'}`}>Direct Answer</h3>
            </div>
          ...
          {response.explanation?.length > 0 && (
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 bg-blue-100 rounded-xl flex items-center justify-center"><BookOpen className="w-4 h-4 text-blue-600" /></div>
                <h3 className="font-bold text-gray-900">Step-by-Step Explanation</h3>
              </div>
          ...
          {response.applications?.length > 0 && (
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 bg-emerald-100 rounded-xl flex items-center justify-center"><Globe className="w-4 h-4 text-emerald-600" /></div>
                <h3 className="font-bold text-gray-900">Real-World Applications</h3>
              </div>
          ...
          <div className={`${usedModel === 'claude' ? 'bg-purple-50 border-purple-100' : 'bg-emerald-50 border-emerald-100'} rounded-2xl p-4 border text-center`}>
            <p className={`text-sm ${usedModel === 'claude' ? 'text-purple-700' : 'text-emerald-700'} mb-3`}>Have another question?</p>
            <button onClick={reset} className={`bg-gradient-to-r ${responseCfg.color} text-white text-sm font-semibold px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity`}>
